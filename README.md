...
オプションで文字起こし（[whisper-rs](https://github.com/tazz4843/whisper-rs) により [whisper.cpp](https://github.com/ggerganov/whisper.cpp) を組み込み済み）とキーフレーム抽出も実行でき、MCP サーバー経由で AI に作業フローを分析させることを主な用途としています。
...
- whisper.cpp（`--transcribe` 使用時）: **なし（組み込み済み）** — モデルファイル（`.bin`）のみ必要
- ビルド時に [CMake](https://cmake.org/) 3.14+ および [LLVM](https://releases.llvm.org/) が必要（whisper-rs のビルド依存）
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `--search <QUERY>` | なし | manifest.json のトランスクリプトを横断検索する（INPUT を manifest.json パスとして扱う）|
